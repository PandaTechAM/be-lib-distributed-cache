...
/// <summary>Registration extensions for wiring the distributed cache into a web application.</summary>
public static class WebApplicationBuilderExtension
{
    /// <summary>Register Redis-backed HybridCache, distributed locking, rate limiting, and a Redis health check.</summary>
    ...
    private static void ValidateOptions(CacheConfigurationOptions options)
    {
        ...
        if (options.DistributedLockMaxDuration <= TimeSpan.FromSeconds(1))
        {
            throw new ArgumentException(
                "AddDistributedCache: DistributedLockMaxDuration must be greater than 1 second.");
        }
